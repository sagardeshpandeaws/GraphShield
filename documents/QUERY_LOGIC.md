...
Each lifecycle finding is pre-enriched, carries an `OWASP NHI` compliance mapping, **cites the actual Entra sign-in records** behind the signal (timestamp, event kinds, credential type, user agent, conditional-access status, contributing feed), and is exported into the same AI-PDF / PDF / Excel / CSV artifacts. When several identities trigger the same finding, their evidence and identity relationships are merged rather than dropped. **Feed absent → these findings are not emitted**; the 80-finding baseline and all exporters are byte-identical.**Specific findings added:** The lifecycle feed (if supplied) adds the eight NHI lifecycle findings listed above (AZ-055 – AZ-062); without a feed, the total stays at **80** exactly. A supplied feed also drives the feed-gated per-identity NHI Lifecycle Dashboard (identity, last sign-in, sign-in count, activity period, credential type, lifecycle stages, attestation / rotation / onboarding / cross-source).
